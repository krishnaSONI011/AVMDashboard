...
function setupDrawnByMeModal() {
    ...
    const page4Container = document.getElementById('page4');
    const drawnByMeBtn = (page5Container && page5Container.querySelector('#drawn-by-me-btn'))
        || (page4Container && page4Container.querySelector('#drawn-by-me-btn'))
        || document.getElementById('drawn-by-me-btn');
    const drawnByMeModal = (page5Container && page5Container.querySelector('#drawn-by-me-modal'))
        || (page4Container && page4Container.querySelector('#drawn-by-me-modal'))
        || document.getElementById('drawn-by-me-modal');
    const drawnByMeTextarea = (page5Container && page5Container.querySelector('#drawn-by-me-textarea'))
        || (page4Container && page4Container.querySelector('#drawn-by-me-textarea'))
        || document.getElementById('drawn-by-me-textarea');
...
function setupPage1Events() {
   ...
   yearInput.value = new Date().getFullYear();
    ...
    // Set up petitioner names display
    setupPetitionerNamesDisplay();
...
function setupPage3Events() {
    // add presently at
    const presently_at_select = document.getElementById("presently_at")
    const presently_at_line = document.getElementById("presently_at_line")

    presently_at_select.addEventListener("change" , (e)=>{
        if(e.target.value === "yes"){
            presently_at_line.classList.remove("hidden")
        }
        else presently_at_line.classList.add("hidden")
    })
...
/**
 * Sets up the petitioner names display functionality
 * Shows a list of all petitioner names entered in page 1
 */

function setupPetitionerNamesDisplay() {
    const petitionerNamesList = document.getElementById('petitioner-names-list');
    if (!petitionerNamesList) return;

    // Function to update the display
    function updatePetitionerNamesDisplay() {
        const petitionerBlocks = document.querySelectorAll('.petitioner_block');
        const names = [];
        ...
        // Update the display
        if (names.length > 0) {
            petitionerNamesList.innerHTML = names.map(name => `<span class="mb-1 text-white">${name}</span> ,`).join('');
        } else {
            petitionerNamesList.innerHTML = '<div class="text-white italic">No petitioner names entered yet</div>';
        }
    }

    // Set up event listeners for all petitioner name inputs
    function setupPetitionerNameListeners() {
        const petitionerBlocks = document.querySelectorAll('.petitioner_block');
        ...
        petitionerBlocks.forEach(block => {
            ...
            if (nameInput) {
                ...
                nameInput.removeEventListener('input', updatePetitionerNamesDisplay);
                // Add new listener
                nameInput.addEventListener('input', updatePetitionerNamesDisplay);
            }
        });
    }

    // Initial setup
    setupPetitionerNameListeners();
    updatePetitionerNamesDisplay();

    // Re-setup listeners when new petitioners are added
    const originalAddPetitioner = document.getElementById('add_petitioner_button');
    if (originalAddPetitioner) {
        originalAddPetitioner.addEventListener('click', () => {
            // Wait a bit for the new block to be added, then re-setup listeners
            setTimeout(() => {
                setupPetitionerNameListeners();
                updatePetitionerNamesDisplay();
            }, 100);
    ...
    // Also update when petitioners are removed
    document.addEventListener('click', (e) => {
        if (e.target.classList.contains('remove_petitioner_btn')) {
            setTimeout(() => {
                setupPetitionerNameListeners();
                updatePetitionerNamesDisplay();
            }, 100);
...
/**
 * Sets up page 5 events and functionality
 * Creates dynamic affidavit sections for each petitioner
 */
function setupPage5Events() {
    // Set up petitioner affidavits display
    setupPetitionerAffidavits();
...
/**
 * Creates individual affidavit sections for each petitioner
...
function setupPetitionerAffidavits() {
    ...
    function updatePetitionerAffidavits() {
        ...
        // Build a single affidavit section with all petitioner full details together
        const petitionerEntries = [];
        petitionerBlocks.forEach((block) => {
            const nameInput = block.querySelector('input[placeholder="Enter Petitioner Name"]');
            const aliasInput = block.querySelector('input[placeholder="Enter Petitioner Alias Name"]');
            const ageInput = block.querySelector('input[name="petitioner_age"]');
            const occupationSelect = block.querySelector('select[name="occupation"]');
            const religionSelect = block.querySelector('select[name="religion"]');
            const citizenshipSelect = block.querySelector('select[name="citizenship"]');
            const addressInput = block.querySelector('input[placeholder="Enter Petitioner Address"]');
            const inhabitantInput = block.querySelector('input[name="petitioner_inhabitent"]');

            const name = nameInput ? nameInput.value.trim() : '';
            const alias = aliasInput ? aliasInput.value.trim() : '';
            const age = ageInput ? ageInput.value.trim() : '';
            const occupation = occupationSelect ? occupationSelect.options[occupationSelect.selectedIndex].text : '';
            const religion = religionSelect ? religionSelect.options[religionSelect.selectedIndex].text : '';
            const citizenship = citizenshipSelect ? citizenshipSelect.options[citizenshipSelect.selectedIndex].text : '';
            const address = addressInput ? addressInput.value.trim() : '';
            const inhabitant = inhabitantInput ? inhabitantInput.value.trim() : '';
            ...
            const inhabitantPart = `, Inhabitant of <span class="font-bold text-white">${inhabitant || 'INHABITANT'}</span>`;
        ...
        const isPlural = petitionerEntries.length > 1;
        const pronoun = isPlural ? 'We' : 'I';
        const petitionerLabel = isPlural ? 'PETITIONERS' : 'PETITIONER';
        const abovenamedLabel = isPlural ? 'the Petitioners' : 'the Petitioner';

        const detailsMarkup = petitionerEntries.length
            ? petitionerEntries.join('; ')
            : '<span class="font-bold text-white">NAME OF PETITIONER</span>';

        const affidavitsHTML = `
            <div class="petitioner-affidavit-section mb-8 border-b border-gray-600 pb-6">
                <p class="text-center mt-10 text-lg font-bold">AFFIDAVIT OF ${petitionerLabel}</p>
                <p class="text-center text-sm">(NO IDENTITY PROOF)</p>
    ...
    function setupPetitionerInputListeners() {
        ...
        petitionerBlocks.forEach(block => {
            // Listen to all input changes in this block
            const inputs = block.querySelectorAll('input, select');
            inputs.forEach(input => {
                input.removeEventListener('input', updatePetitionerAffidavits);
                input.removeEventListener('change', updatePetitionerAffidavits);
                input.addEventListener('input', updatePetitionerAffidavits);
                input.addEventListener('change', updatePetitionerAffidavits);
            });
        });
    }
    ...
    // Initial setup
    setupPetitionerInputListeners();
    updatePetitionerAffidavits();

    // Re-setup listeners when petitioners are added/removed
    document.addEventListener('click', (e) => {
        if (e.target.id === 'add_petitioner_button' || e.target.classList.contains('remove_petitioner_btn')) {
            setTimeout(() => {
                setupPetitionerInputListeners();
